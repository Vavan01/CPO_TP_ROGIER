...
public String toString() {
    ...
    for (int x = 0; x < nbLignes; x++) {
        ...
        for (int y = 0; y < nbColonnes; y++) {
            Etat etatCellule = matriceCellules[x][y].getEtat();
            String symbol = "O";
            if (etatCellule == Etat.Allume) {
                symbol = "X";
            } else if (etatCellule == Etat.Allume) {
                symbol = "O";
            } else if (etatCellule == Etat.Inter){
                symbol = "A"; // Remplacez cela par le symbole correspondant au troisième état
            }
            gridString.append(" " + symbol + " |");
        }
        gridString.append("\n");
    }
    gridString.append("-------------------------------\n");
